export const StarsReview: React.FC<{ rating: number, size: number }> = (props) => {
    ...
    if (rating !== undefined && rating > 0 && rating <= 5) {
        for (let i = 0; i <= 4; i++) {
            // if there are whole positive stars rem
            // then increase the fullstars and decrease the 
            // rating
            if (rating - 1 >= 0) {
                fullStars = fullStars + 1;
                rating = rating - 1;
                // if there are .5 stars rem
                // increase the half
            } else if (rating === .5) {
                halfStars = halfStars + 1;
                rating = rating - .5;
                // when its zero increase the emptystars
            } else if (rating === 0) {
    ...
    return (
        <div>
            {/* Arrays.from{{length : arg1}, (arg2, arg3) => {

            }} 
                    @arg1 -> is the length until the seq data is getting mapped
                    @arg2 -> is the key
                    @arg3 -> is the value
